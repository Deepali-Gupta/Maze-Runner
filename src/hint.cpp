...
const GLfloat START_Y_AT=5.0f;
const GLfloat START_ROT=270.0f;
const GLfloat START_CAMERA_Y=15.0f;
...
void print_maze() //Renders the necessary OpenGL cubes
{
 ...
 for(y=0; y<YSIZE ; ++y ) //Maze proper
 {
  for(x=0; x<XSIZE ; ++x )
  {
   ...
   else if((maze_innards())[x][y]!=NO_PATH && visited[x][y]==0 && (x!=XSIZE/2 || y!=YSIZE/2))
   {
	//printf("%d\n",visited[x][y]);
   ...
   if((maze_innards())[x][y]!=NO_PATH && visited[x][y]==0 && x==XSIZE/2 && y==YSIZE/2)
   {
	//printf("%d\n",visited[x][y]);
...
{ 
 ...
 /*if(!coin_obj.load("models/coin2.obj"))
            {
            	cout<<"could not load the coin2 obj"<<endl;
            }
*/
